...
class Veterinalia_Appointment_Manager {
    ...
    private $crm_db_handler; // Manejador de la base de datos CRM
    ...
    private function __construct() {
        ...
        $this->crm_db_handler = Veterinalia_CRM_Database::get_instance();
...
public function enqueue_assets() {
    ...
    public function save_professional_schedule( $professional_id, $schedule_data ) {
        ...
        $this->db_handler->delete_professional_availability( $professional_id );

        // 2. Insertar nuevos horarios
        $success = true;
        foreach ( $schedule_data as $schedule_item ) {
            $dia_semana_id = intval( $schedule_item['dia_semana_id'] );
            $start_time = sanitize_text_field( $schedule_item['start_time'] );
            $end_time = sanitize_text_field( $schedule_item['end_time'] );
            $slot_duration = intval( $schedule_item['slot_duration'] );

            if ( ! $this->db_handler->insert_professional_availability( $professional_id, $dia_semana_id, $start_time, $end_time, $slot_duration ) ) {
                $success = false;
            }
        }
    ...
    public function get_professional_schedule( $professional_id ) {
        ...
        $schedule = $this->db_handler->get_professional_availability( $professional_id );
 ...
 * @param int $service_duration DuraciÃƒÂ³n del servicio que se quiere reservar, en minutos.
...
public function get_available_slots_for_date( $professional_id, $selected_date, $service_duration ) {
    ...
    // 1. Obtener horario de trabajo e intervalo de presentaciÃƒÂ³n.
    $dia_semana_id = date( 'N', strtotime( $selected_date ) );
    $working_hours_rules = $this->db_handler->get_professional_availability( $professional_id );
    ...
    foreach ( $working_hours_rules as $rule ) {
        if ( $rule->dia_semana_id == $dia_semana_id ) {
            $todays_working_blocks[] = [
                'start' => strtotime( $selected_date . ' ' . $rule->start_time ),
                'end'   => strtotime( $selected_date . ' ' . $rule->end_time )
            ];
            // El campo 'slot_duration' ahora se reutiliza como el intervalo de presentaciÃƒÂ³n.
            $presentation_interval = intval( $rule->slot_duration );
        }
    }

    if ( empty( $todays_working_blocks ) ) {
        return []; // El profesional no trabaja este dÃƒÂ­a.
    }

    // 2. Obtener citas ya reservadas (esta consulta es rÃƒÂ¡pida gracias al ÃƒÂ­ndice en la BD).
    $booked_appointments = $this->db_handler->get_appointments_for_date( $professional_id, $selected_date );
    ...
    // 3. Algoritmo HÃƒÂ­brido
    $available_slots = [];
    $duration_in_seconds = $service_duration * 60;
    $interval_in_seconds = $presentation_interval * 60;

    foreach ( $todays_working_blocks as $block ) {
        // El "cursor" es un puntero de tiempo que se moverÃƒÂ¡ a travÃƒÂ©s del dÃƒÂ­a.
        $cursor = $block['start'];

        while ( $cursor < $block['end'] ) {
            // Definimos el slot potencial que queremos verificar.
            $potential_slot_start = $cursor;
            $potential_slot_end = $potential_slot_start + $duration_in_seconds;

            // Si el slot potencial termina despuÃƒÂ©s de la jornada laboral, nos detenemos.
            if ( $potential_slot_end > $block['end'] ) {
                break;
            }

            // Verificamos si este slot potencial choca con alguna cita ya agendada.
            $is_available = true;
            foreach ( $busy_slots as $busy_slot ) {
                // CondiciÃƒÂ³n de solapamiento: un slot estÃƒÂ¡ ocupado si empieza antes de que termine una cita
                // y termina despuÃƒÂ©s de que esa misma cita haya comenzado.
                if ( $potential_slot_start < $busy_slot['end'] && $potential_slot_end > $busy_slot['start'] ) {
                    $is_available = false;
                    // Si encontramos una colisiÃƒÂ³n, movemos nuestro cursor inteligentemente al final
                    // de la cita que causa el conflicto para no seguir buscando en un espacio ya ocupado.
            ...
            if ( $is_available ) {
                // Ã‚Â¡Encontramos un hueco! Lo aÃƒÂ±adimos a la lista de horarios disponibles.
                $available_slots[] = date( 'H:i', $potential_slot_start );

                // --- La magia del intervalo de presentaciÃƒÂ³n ---
                // Movemos el cursor al siguiente punto de inicio definido por el intervalo del profesional.
                $cursor += $interval_in_seconds;
            }
        }
    }

    // Eliminamos duplicados (por si alguna lÃƒÂ³gica compleja los generara) y ordenamos el resultado final.
...
public function book_appointment( $booking_data ) {
    ...
    if (!empty($client_email)) {
        ...
        $existing_client = $this->crm_db_handler->get_client_by_email($client_email);
        ...
        if ($existing_client) {
            ...
            $client_id = $this->crm_db_handler->create_guest_client($client_data);
            if (!$client_id) {
        ...
        if ( $wp_user_id ) {
            ...
            $crm_client = $this->crm_db_handler->get_client_by_user_id( $wp_user_id );
            if ( $crm_client ) {
    ...
    if (!empty($pet_name) && $client_id) {
        ...
        $existing_pet = $this->crm_db_handler->get_pet_by_name_and_client($pet_name, $client_id);
        ...
        if ($existing_pet) {
            ...
            if (!empty($pet_species) || !empty($pet_breed) || isset($booking_data['pet_gender'])) {
                ...
                if (!empty($update_data)) {
                    $this->crm_db_handler->update_pet($pet_id, $update_data);
            ...
            $pet_id = $this->crm_db_handler->create_pet_with_share_code($pet_data);
        }
        if (!$pet_id) {
    ...
    if (!empty($booking_data['pet_id']) && !empty($client_id)) {
        ...
        $pet = $this->crm_db_handler->get_pet_by_id($booking_data['pet_id']);
        if (!$pet) {
    ...
    try {
        ...
        $is_slot_taken = $this->db_handler->is_slot_already_booked(
        ...
        $inserted = $this->db_handler->insert_appointment($data_to_insert);
    ...
    public function get_professional_appointments( $professional_id, $args = [] ) {
        ...
        $appointments = $this->db_handler->get_appointments_by_professional_id( $professional_id, $args );
    ...
    public function update_appointment_status( $appointment_id, $new_status, $professional_id = 0 ) {
        ...
        $updated = $this->db_handler->update_appointment_status( $appointment_id, $new_status );
        ...
        if ( $updated ) {
            ...
            global $wpdb;
            $table_name_appointments = $wpdb->prefix . 'va_appointments';
            $appointment_from_db = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table_name_appointments} WHERE id = %d", $appointment_id ) );
...
public function get_services_for_category_api( $request ) {
    ...
    public function get_professional_availability_for_range($professional_id, $date_from, $date_to) {
        ...
        $query = "
            ...
            AND is_available = 1
            ORDER BY dia_semana_id ASC
        ";
        
        $schedule = $wpdb->get_results($wpdb->prepare($query, [$professional_id]));
        ...
        while ($current_date <= $end_date) {
            ...
            // Buscar horario para este dÃƒÂ­a
            $day_schedule = null;
            foreach ($schedule as $slot) {
                if (strtolower($slot->day_of_week) === $day_of_week) {
                    $day_schedule = $slot;
                    break;
                }
            }
    ...
    public function create_appointment_from_agenda($appointment_data) {
        ...
        if (!empty($appointment_data['client_id'])) {
            ...
            // Obtener la mascota para verificar su client_id
            $pet = $this->crm_db_handler->get_pet_by_id($appointment_data['pet_id']);
            if (!$pet) {
        ...
        global $wpdb;
        $table_appointments = $wpdb->prefix . 'va_appointments';
        
        error_log('[Veterinalia Manager] Insertando cita en tabla: ' . $table_appointments);
        error_log('[Veterinalia Manager] Datos a insertar: ' . print_r($insert_data, true));
        
        $result = $wpdb->insert($table_appointments, $insert_data);
        
        if ($result === false) {
            error_log('[Veterinalia Manager] Error en inserciÃƒÂ³n: ' . $wpdb->last_error);
            return [
                'success' => false,
                'message' => 'Error al crear la cita en la base de datos: ' . $wpdb->last_error
            ];
        }
    ...
    private function check_time_slot_availability($professional_id, $start_time, $end_time) {
        ...
        $query = "
            ...
            AND (
                (appointment_start < %s AND appointment_end > %s) OR
                (appointment_start < %s AND appointment_end > %s) OR
                (appointment_start >= %s AND appointment_end <= %s)
            )
        ";
        ...
        error_log('[Veterinalia Manager] Consulta de disponibilidad: ' . $prepared_query);
        
        $count = $wpdb->get_var($prepared_query);
        
        error_log('[Veterinalia Manager] Citas solapadas encontradas: ' . $count);
        
        $available = $count == 0;
        error_log('[Veterinalia Manager] Slot disponible: ' . ($available ? 'SÃƒÂ' : 'NO'));
    ...
    public function complete_appointment_with_log($log_data) {
        ...
        $db_handler = Veterinalia_CRM_Database::get_instance();

        // 1. Guardar la entrada en la bitÃƒÂ¡cora
        $log_id = $db_handler->create_pet_log($log_data);
    ...
    private function send_booking_emails($appointment_id, $is_new_client, $client_id, $pet_id, $booking_data) {
        ...
        try {
            ...
            if ($is_new_client && $pet_id) {
                ...
                $pet = $this->crm_db_handler->get_pet_by_id($pet_id);
    ...
    private function get_or_create_client_id_for_current_user() {
        ...
        if (is_user_logged_in() && class_exists('Veterinalia_CRM_Database')) {
            $user_id = get_current_user_id();
            $crm = Veterinalia_CRM_Database::get_instance();
            
            if (method_exists($crm, 'get_client_by_user_id')) {
                $client_obj = $crm->get_client_by_user_id($user_id);
                ...
                if (!empty($client_obj) && !empty($client_obj->client_id)) {
                    ...
                    if ($user_type === 'author') {
                        ...
                        return 0; // No crear cliente para profesionales
                        ...
                        return 0; // Solo crear clientes para usuarios 'general'
                    }
                    
                    // Cliente no existe y es usuario general, crearlo automáticamente
                    error_log('[Appointment Manager] Usuario ' . $user_id . ' tiene _user_type=general y no tiene registro. Creando automáticamente...');
                    
                    $wp_user = get_user_by('id', $user_id);
                    if ($wp_user && method_exists($crm, 'create_client')) {
                        $client_data = [
                            'user_id' => $user_id,
                            'name' => $wp_user->display_name ?: $wp_user->user_login,
                            'email' => $wp_user->user_email,
                            'notes' => 'Cliente creado automáticamente al acceder al dashboard'
                        ];
                        
                        $new_client_id = $crm->create_client($client_data);
                        if ($new_client_id) {
                            $client_id_val = intval($new_client_id);
