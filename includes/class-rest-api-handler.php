...
class Veterinalia_Appointment_REST_Handler
{
    ...
    public function handle_client_search(WP_REST_Request $request) {
        ...
        try {
            ...
            $clients = $db_handler->search_clients_with_access_check($term, $professional_id);
            if ($clients === null) {
                error_log('[Chocovainilla] API: Fallback de búsqueda básico activado.');
                $clients = $db_handler->search_clients_basic($term);
    ...
    public function handle_grant_pet_access(WP_REST_Request $request) {
        ...
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $pet = $db_handler->get_pet_by_id($pet_id);
            ...
            $access_granted = $db_handler->grant_pet_access($pet_id, $professional_id, 'full');
    ...
    public function handle_get_pets_by_client($request) {
        ...
        try {
            ...
            if (!empty($professional_id)) {
                ...
                $pets = $db_handler->get_pets_by_client_with_access($client_id, $professional_id);
            } else {
                error_log('[Chocovainilla] API: get_pets_by_client client_id=' . $client_id);
                $pets = $db_handler->get_pets_by_client($client_id);
            }
    ...
    public function handle_get_clients(WP_REST_Request $request)
    {
        ...
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $clients = $db_handler->get_clients_by_professional($professional_id);
            
            // Adjuntar mascotas a cada cliente
            foreach ($clients as $client) {
                $client->pets = $db_handler->get_pets_by_client($client->client_id);
            }
    ...
    public function handle_create_client(WP_REST_Request $request)
    {
        ...
        try {
            ...
            // Crear el cliente
            $client_id = $db_handler->create_client($client_data);
            ...
            $new_client = $db_handler->get_client_by_id($client_id);
    ...
    public function handle_create_pet(WP_REST_Request $request)
    {
        ...
        try {
            ...
            // 1. Crear la mascota (lógica existente)
            $pet_id = $db_handler->create_pet($params);
            if (!$pet_id) {
                return new WP_Error('create_failed', 'No se pudo crear la mascota.', ['status' => 500]);
            }

            // 2. Otorgar acceso al profesional que la creó (lógica existente)
            $db_handler->grant_pet_access($pet_id, $professional_id, 'full');

            // 3. OBTENER DATOS Y DISPARAR CORREO SI ES INVITADO
            $client = $db_handler->get_client_by_id($client_id);
            $pet = $db_handler->get_pet_by_id($pet_id);
    ...
    public function handle_import_pet(WP_REST_Request $request)
    {
        ...
        try {
            ...
            // 1. Encontrar la mascota por el código
            $pet = $db_handler->get_pet_by_share_code($share_code);
            if (!$pet) {
                return new WP_Error('not_found', 'Código no válido o no encontrado', ['status' => 404]);
            }

            // 2. Verificar si ya tiene acceso
            $has_access = $db_handler->check_pet_access($professional_id, $pet->pet_id);
            if ($has_access) {
                return new WP_Error('already_imported', 'Este paciente ya está en tu lista', ['status' => 409]);
            }

            // 3. Otorgar acceso
            $access_granted = $db_handler->grant_pet_access($pet->pet_id, $professional_id, 'full');
            if (!$access_granted) {
                return new WP_Error('grant_failed', 'No se pudo otorgar acceso a la mascota', ['status' => 500]);
            }

            // 4. Obtener los datos del cliente para la respuesta
            $client = $db_handler->get_client_by_id($pet->client_id);
    ...
    public function handle_get_pet_logs(WP_REST_Request $request) {
        ...
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $logs = $db_handler->get_pet_logs_full($pet_id, $professional_id);
    ...
    public function handle_update_pet(WP_REST_Request $request)
    {
        ...
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $result = $db_handler->update_pet($pet_id, $params);
    ...
    public function check_pet_permission(WP_REST_Request $request)
    {
        ...
        $db_handler = Veterinalia_CRM_Database::get_instance();
        $has_access = $db_handler->check_pet_access($professional_id, $pet_id);
        ...
        if (!$has_access) {
            ...
            if (!empty($listings)) {
                ...
                $has_access = $db_handler->check_pet_access($listing_id, $pet_id);
    ...
    public function handle_claim_profile(WP_REST_Request $request) {
        ...
        try {
            ...
            $linked = $db_handler->link_client_to_user($pet->client_id, $user_id);
    ...
    public function handle_create_pet_log(WP_REST_Request $request) {
        ...
        try {
            ...
            foreach ($meta_data as $key => $value) {
                $crm_db->add_pet_log_meta($log_id, sanitize_key($key), sanitize_textarea_field($value));
            }

            // 3. Guardar los productos utilizados
            foreach ($products_data as $product) {
                $crm_db->add_pet_log_product($log_id, intval($product['product_id']), $product);
            }
    ...
    public function handle_get_my_pets(WP_REST_Request $request) {
        try {
            ...
            if (!$client) {
                ...
                if (!$wp_user) {
                    error_log('[REST API] Usuario de WordPress no encontrado para user_id: ' . $user_id);
                    return new WP_REST_Response(['success' => true, 'data' => []], 200);
                }
                
                // Crear el cliente automáticamente con los datos del usuario de WordPress
                $client_data = [
                    'user_id' => $user_id,
                ...
                if (!$client) {
                    error_log('[REST API] No se pudo obtener el cliente recién creado en get_my_pets');
                    return new WP_REST_Response(['success' => true, 'data' => []], 200);
                }
                
                error_log('[REST API] Cliente creado automáticamente con ID: ' . $client_id . ' para user_id: ' . $user_id . ' en get_my_pets');
            }
            
            $pets = $db->get_pets_by_client($client->client_id);
        ...
        try {
            ...
            if (!$client) {
                ...
                $client_data = [
                    ...
                    'name' => $wp_user->display_name ?: $wp_user->user_login,
                    'email' => $wp_user->user_email,
                    'notes' => 'Cliente creado automáticamente al registrar primera mascota'
                ];
                ...
                $client = $db->get_client_by_id($client_id);
                if (!$client) {
                    return new WP_Error('client_retrieval_failed', 'No se pudo obtener el cliente recién creado', ['status' => 500]);
                }
                
                error_log('[REST API] Cliente creado automáticamente con ID: ' . $client_id . ' para user_id: ' . $user_id);
            }
            // Dedupe por nombre (case-insensitive) para el cliente
            $existing = $db->get_pets_by_client($client->client_id);
            ...
            $pet_id = $db->create_pet_with_share_code([
    ...
    public function check_client_pet_permission(WP_REST_Request $request) {
        ...
        try {
            $db = Veterinalia_CRM_Database::get_instance();
            $pet = $db->get_pet_by_id($pet_id);
            $client = $db->get_client_by_user_id(get_current_user_id());
    ...
    public function handle_get_client_pet_logs(WP_REST_Request $request) {
        ...
        try {
            $db = Veterinalia_CRM_Database::get_instance();
            $logs = $db->get_pet_logs($pet_id, null);
            return new WP_REST_Response(['success' => true, 'data' => $logs ?: []], 200);
        } catch (Exception $e) {
    ...
    public function handle_get_client_pet_summary(WP_REST_Request $request) {
        ...
        try {
            ...
            $appointments_table = esc_sql($wpdb->prefix . 'va_appointments');
            $next_appointment = null;
            $sql = $wpdb->prepare("SELECT appointment_start FROM {$appointments_table} WHERE pet_id = %d AND appointment_start > NOW() AND status IN ('pending','confirmed') ORDER BY appointment_start ASC LIMIT 1", $pet_id);
            $dt = $wpdb->get_var($sql);
            if ($dt) { $next_appointment = mysql2date('d M, H:i', $dt); }
            return new WP_REST_Response(['success' => true, 'data' => [
                'next_appointment' => $next_appointment ?: 'Ninguna',
    ...
    public function handle_get_client_notifications(WP_REST_Request $request) {
        ...
        try {
            ...
            $items = [];
            foreach ($pets as $p) {
                $sql = $wpdb->prepare("SELECT appointment_start FROM {$appointments_table} WHERE pet_id = %d AND appointment_start > NOW() AND status IN ('pending','confirmed') ORDER BY appointment_start ASC LIMIT 1", intval($p->pet_id));
                $dt = $wpdb->get_var($sql);
                if ($dt) {
                    $items[] = [ 'pet_id' => intval($p->pet_id), 'pet_name' => $p->name, 'when' => mysql2date('d M, H:i', $dt) ];
                }
            }
    ...
    public function handle_get_entry_types(WP_REST_Request $request) {
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $entry_types = $db_handler->get_entry_types();
    ...
    public function handle_get_form_fields(WP_REST_Request $request) {
        ...
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $form_fields = $db_handler->get_form_fields_by_entry_type($entry_type_id);
    ...
    public function handle_get_products(WP_REST_Request $request) {
        $professional_id = intval($request->get_param('professional_id'));
        $db_handler = Veterinalia_CRM_Database::get_instance();
        $products = $db_handler->get_products_by_professional($professional_id);
    ...
    public function handle_save_product(WP_REST_Request $request) {
        ...
        $db_handler = Veterinalia_CRM_Database::get_instance();
        $saved_id = $db_handler->save_product($params);
    ...
    public function handle_delete_product(WP_REST_Request $request) {
        ...
        $db_handler = Veterinalia_CRM_Database::get_instance();
        $deleted = $db_handler->delete_product($product_id, $professional_id);
    ...
    public function handle_get_manufacturers(WP_REST_Request $request) {
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $manufacturers = $db_handler->get_manufacturers();
    ...
    public function handle_get_active_ingredients(WP_REST_Request $request) {
        try {
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $active_ingredients = $db_handler->get_active_ingredients();
    ...
    public function handle_get_products_full(WP_REST_Request $request) {
        try {
            ...
            $db_handler = Veterinalia_CRM_Database::get_instance();
            $products = $db_handler->get_products_full($professional_id);
            return new WP_REST_Response(['success' => true, 'data' => $products], 200);
        } catch (Exception $e) {
