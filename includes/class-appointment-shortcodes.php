...
class Veterinalia_Appointment_Shortcodes {
    ...
    public function render_client_booking_form($atts) {
        ...
        if ($current_user_id && class_exists('Veterinalia_CRM_Database')) {
            $crm_db = Veterinalia_CRM_Database::get_instance();
            $client = $crm_db->get_client_by_user_id($current_user_id);

            if ($client) {
                $client_pets = $crm_db->get_pets_by_client($client->client_id);
   ...
   public function render_professional_dashboard( $atts ) {
    ...
    if ( $user_type === 'author' ) {
        ...
        error_log('[Dashboard] Usuario ' . $current_user_id . ' tiene _user_type=author. Mostrando dashboard profesional.');
        
        // Obtener listados para la plantilla
        $db_handler = Veterinalia_Appointment_Database::get_instance();
        $professional_listings = $db_handler->get_listings_by_author_id( $current_user_id );
    ...
    error_log('[Dashboard] Usuario ' . $current_user_id . ' tiene _user_type=' . $user_type . '. Mostrando dashboard de cliente.');
    ...
    public function render_client_dashboard($atts) {
        ...
        if (!wp_script_is('va-client-dashboard-v2', 'enqueued')) {
            ...
            if ($user_type === 'general') {
                ...
                error_log('[Appointment Shortcodes] Usuario tiene _user_type=' . $user_type . ', no se obtiene client_id para localize.');
            }
        ...
        if ($current_user_id) {
            ...
            // Solo obtener cliente si es usuario general
            $user_type = get_user_meta($current_user_id, '_user_type', true);
            if ($user_type === 'general') {
                // Obtener el ID de cliente asociado al usuario de WordPress
                $client = $db_handler->get_client_by_user_id($current_user_id);

                if ($client) {
                    $client_id = $client->client_id;
                    // Obtener las mascotas vinculadas a este cliente
                    $client_pets = $db_handler->get_pets_by_client($client_id);
                }
            } else {
                error_log('[Appointment Shortcodes] Usuario tiene _user_type=' . $user_type . ', no se obtienen datos de cliente para la plantilla.');
            }
        }
        ...
        if (is_user_logged_in() && class_exists('Veterinalia_CRM_Database')) {
            ...
            if (method_exists($crm, 'get_client_by_user_id')) {
                ...
                if (!empty($client_obj) && !empty($client_obj->client_id)) {
                    ...
                    if ($user_type === 'author') {
                        error_log('[Appointment Shortcodes] Usuario ' . $user_id . ' tiene _user_type=author. NO se crea registro de cliente.');
                        return 0; // No crear cliente para profesionales
                    } elseif ($user_type !== 'general') {
                        error_log('[Appointment Shortcodes] Usuario ' . $user_id . ' tiene _user_type=' . $user_type . ' (no es general). NO se crea registro de cliente.');
                        return 0; // Solo crear clientes para usuarios 'general'
                    }
                    
                    // Cliente no existe y es usuario general, crearlo automáticamente
                    error_log('[Appointment Shortcodes] Usuario ' . $user_id . ' tiene _user_type=general y no tiene registro. Creando automáticamente...');
                    ...
                    if ($wp_user && method_exists($crm, 'create_client')) {
                        ...
                        if ($new_client_id) {
                            ...
                            error_log('[Appointment Shortcodes] Cliente creado automáticamente con ID: ' . $client_id_val . ' para user_id: ' . $user_id);
                        } else {
                            error_log('[Appointment Shortcodes] Error al crear cliente automático para user_id: ' . $user_id);
                        }
                    }
                }
            }
