...
class Veterinalia_Appointment_AJAX_Handler {
    ...
    public function handle_get_availability_for_range() {
        ...
        while ($current_date_obj <= $end_date_obj) {
            ...
            $slots = $manager->get_available_slots_for_date($professional_id, $current_date_str, $service_duration);

            $formatted_slots = [];
            foreach ($slots as $slot_time) {
                $formatted_slots[] = ['time' => $slot_time, 'status' => 'available'];
            }

            $availability_data[$current_date_str] = $formatted_slots;
