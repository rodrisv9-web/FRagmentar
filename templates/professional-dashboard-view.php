                                    ...
                                    <?php if ( ! empty( $professional_listings ) ) : ?>
                                        <div id="employee-list" class="employees-container">
                                            <?php foreach ( $professional_listings as $listing ) : ?>
                                                <?php
                                                $title = esc_html( $listing->post_title );
                                                $words = explode( ' ', $title );
                                                $initials = ( count( $words ) >= 2 ) 
                                                    ? strtoupper( substr( $words[0], 0, 1 ) . substr( $words[1], 0, 1 ) ) 
                                                    : strtoupper( substr( $words[0], 0, 2 ) );
                                                ?>
                                                <div class="employee-item" data-listing-id="<?php echo esc_attr( $listing->ID ); ?>">
                                                    <img src="https://placehold.co/120x120/D6BCFA/805AD5?text=<?php echo urlencode( $initials ); ?>" alt="Empleado <?php echo esc_attr( $title ); ?>" class="avatar" loading="lazy" decoding="async" width="120" height="120">
                                                    <p class="employee-name" title="<?php echo esc_attr( $title ); ?>"><?php echo $title; ?></p>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php else : ?>
